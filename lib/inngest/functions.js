...
async function generateFinancialInsights(stats, month) {
  ...
  Analyze the user's monthly investment activity and generate 3 clear, friendly, and actionable insights based on the following data:
  
  - Total Buy Amount (money spent on investments): ₹${stats.totalBuy}
  - Total Sell Amount (money earned from sales): ₹${stats.totalSell}
  - Number of Transactions: ${stats.transactionCount}
  - Asset Breakdown: ${Object.entries(stats.byAssetName)
      .map(([name, value]) => `${name}: $${value}`)
      .join(", ")}
  
  Focus on identifying useful patterns (e.g., over-concentration in one asset, frequent buys without selling, or vice versa), suggest risk balancing or diversification if needed, and keep the tone helpful and easy to understand.
  ...
  try {
    ...
    return [
      "Diversifying your investments across different asset types can help reduce risk and improve long-term returns.",
      "Review your portfolio regularly to ensure it aligns with your financial goals and risk tolerance.",
      "Consider setting up automatic recurring investments to build wealth steadily over time."
    ]
  }
}
  ...
  return transactions.reduce(
    (stats, t) => {
      ...
      if (t.transactionType === "BUY") {
        stats.totalBuy += amount;
        stats.byAssetName[t.assetName] = (stats.byAssetName[t.assetName] || 0) + amount;
      } else {
        stats.totalSell += amount;
      }
      // console.log(stats.byAssetName);
      return stats;
    },
    {
      totalBuy: 0,
